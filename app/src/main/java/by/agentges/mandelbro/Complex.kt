...
operator fun Number.div(c: Complex) = this * Complex.ONE / c
...
class Complex(val real: Double, val img: Double) {
    ...
    override fun toString(): String {
        return when {
            isAlmostZero(img) -> "$real"
            isAlmostZero(real) -> "${img}i"
            img < 0 -> "$real-${-img}i"
            else -> "${real}+${img}i"
        }
    }

    private fun isAlmostZero(d: Double) = abs(d) < DEFAULT_TOLERANCE
